...
def content_loss(content_weight, content_current, content_target):
    ...
    _, C_l, _, _ = content_target.shape
    F = content_current.squeeze().view(C_l, -1)
    P = content_target.squeeze().view(C_l, -1)
    content_loss = content_weight * torch.sum((F - P)**2)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer in enumerate(style_layers):
    	...
    	loss = style_weights[idx] * torch.sum((gram_current - style_targets[idx])**2)
    	style_loss += loss
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    loss = 0
    loss += torch.sum((img[:,:,1:,:] - img[:,:,:-1,:] )**2) # first triple sum (left)
    loss += torch.sum((img[:,:,:,1:] - img[:,:,:,:-1] )**2) # second triple sum (right)
    loss *= tv_weight
